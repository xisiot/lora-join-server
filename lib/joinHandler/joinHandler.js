...
class JoinHandler {
    ...
    .then((res) => {
      if (res.AppKey) {
        ...
        if(res.NwkKey){
          ...
          this.NwkKey = res.AppKey;
        }
        if(res.JoinEUI){
          this.JoinEUI = Buffer.from(res.JoinEUI,'hex');
        } 
  ...
  handler (rxpk) {
    ...
    const RejoinType = joinReq.RejoinType;
    let JoinReqType;
    if(joinReqMHDR.MType === 6){
      if(RejoinType.equals(consts.RejoinType_0)){
        JoinReqType = consts.RejoinType_0;
      } else if(RejoinType.equals(consts.RejoinType_1)){
    ...
    let appKeyQueryOpt;
    //appsKeyQueryOpt is for update DeviceInfo
    const appsKeyQueryOpt= {
      DevEUI: joinReq.DevEUI,
    };
    if(joinReq.JoinEUI){
      appKeyQueryOpt = {
    ...
    const updateDevInfo = (DevAddr) => {
      ...
      return this.DeviceInfo.updateItem(appsKeyQueryOpt, deviceInfoUpd)
